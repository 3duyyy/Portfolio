...
export default function ContactSection() {
  return (
    <section id="contact" className="relative overflow-hidden py-24 sm:py-32">
      ...
      {/* Decorative blobs */}
      <div className="contact-ambient pointer-events-none absolute -top-24 left-1/4 h-72 w-72 rounded-full bg-sky-500/10 blur-3xl" />
      ...
      <div className="container mx-auto px-6 lg:px-12">
        <FadeIn className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="mt-3 text-3xl font-bold sm:text-5xl">
            <TextReveal text="Cùng kết nối nhé!" />
          </h2>
          <FadeIn delay={0.12} y={12}>
            <p className="mt-4 text-zinc-400">
              Muốn hợp tác, trao đổi công nghệ hay chỉ đơn giản là chào hỏi? Chọn kênh bạn thấy thuận tiện — mình sẽ
        ...
        <ScrollReveal className="mx-auto mt-14 grid max-w-5xl grid-cols-1 gap-5 sm:grid-cols-2" stagger={0.09}>
          {channels.map((c) => (
...
function ChannelCard({ channel }: { channel: Channel }) {
  ...
  return (
    <a
      ...
      className={`depth-card group relative flex items-center gap-5 overflow-hidden rounded-2xl border border-white/10 bg-zinc-900/50 p-5 backdrop-blur transition ${channel.ring}`}
    >
      {/* Hover gradient glow */}
      <div
      ...
      <div
        className={`contact-icon flex h-14 w-14 shrink-0 items-center justify-center rounded-xl border border-white/10 bg-zinc-950/60 ${channel.accent}`}
      >
        <Icon size={26} />
      </div>
